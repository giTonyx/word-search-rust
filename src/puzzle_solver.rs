...
impl PuzzleSolver {
    pub fn from_string(input: String) -> PuzzleSolver {
        PuzzleSolver {
    ...
    fn mark_location(location: &Location, len: usize, marked_cells: &mut HashSet<(usize, usize)>) {
        for offset in 0..len {
            if let Some((x, y)) = PuzzleSolver::location_step(location, offset) {
    ...
    fn location_step(location: &Location, step: usize) -> Option<(usize, usize)> {
        let maybe_x = PuzzleSolver::position_with_offset(location.x, location.dx, step);
        let maybe_y = PuzzleSolver::position_with_offset(location.y, location.dy, step);
    ...
    fn search_and_mark(
        ...
        PuzzleSolver::mark_location(&locations[0], word.len(), marked_cells);
        true
    }
